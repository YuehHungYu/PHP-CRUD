...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {	
	if (empty($_GET['id'])) {
		$stmt = $conn->prepare("SELECT * FROM todo ORDER BY id DESC;");
	}
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($method === "POST") {
		...
		// 插入
		$stmt = $conn->prepare("INSERT INTO todo(`content`) VALUES (?);");
		$stmt->bindParam("s", $content);
	}
	...
	if ($method === "PATCH") { // 修改
		...
		if (isset($_POST['content']) && !empty($_POST['content'])) {
			$content = $_POST['content'];
			// 修改內容
			$stmt = $conn->prepare("UPDATE todo SET `content` = ? WHERE id = ?");
			$stmt->bindParam("si", $content, $id);
		}
